...
class SECURITY_ATTRIBUTES(ctypes.Structure):
    _fields_ = [("nLength", DWORD), ("lpSecurityDescriptor",
...
CreateRemoteThread.argtypes = [HANDLE, LPSECURITY_ATTRIBUTES,
...
def find_pvz():
    ...
    if not open_process_by_window("MainWindow", "Plants vs. Zombies"):
        if not open_process_by_window("MainWindow", "植物大战僵尸中文版"):
...
def read_memory(data_type, *address, array=1):
    ...
    for i in range(level):
        ...
        if i != level - 1:
            ...
            success = ReadProcessMemory(pvz_handle, offset, ctypes.byref(
            ...
            success = ReadProcessMemory(pvz_handle, offset, ctypes.byref(
                buff), size, ctypes.byref(bytes_read))
...
def write_memory(data_type, values, *address):
    ...
    for i in range(level):
        ...
        if i != level - 1:
            ...
            success = WriteProcessMemory(pvz_handle, offset, ctypes.byref(
                buff), size, ctypes.byref(bytes_written))
...
def asm_code_inject():
    ...
    # thread_addr = LPVOID()
    thread_addr = VirtualAllocEx(
        pvz_handle, None, size, MEM_COMMIT, PAGE_EXECUTE_READWRITE)

    if thread_addr is not None:

        bytes_written = ctypes.c_ulong()
        success = WriteProcessMemory(
            pvz_handle, thread_addr, asm_code, size, ctypes.byref(bytes_written))

        # thread_handle = HANDLE()
        start = LPTHREAD_START_ROUTINE(thread_addr)
        thread_handle = CreateRemoteThread(
            pvz_handle, None, 0, start, None, 0, None)
...
seeds_string = [
    ...
    ["Puff-shroom", "小喷菇", "小喷", "喷汽蘑菇", "烟雾蘑菇",
        "免费蘑菇", "炮灰菇", "小蘑菇", "免费货", "免费", "紫蘑菇"],
    ...
    ["Ice-shroom", "冰川菇", "寒冰菇", "冰菇", "冷冻蘑菇",
        "冰蘑菇", "冰莲菇", "面瘫", "蓝冰", "原版冰", "冰"],
    ["Doom-shroom", "末日菇", "毁灭菇", "核蘑菇", "核弹",
        "核武", "毁灭", "末日蘑菇", "末日", "黑核", "原版核", "核"],
    ["Lily Pad", "莲叶", "睡莲", "荷叶", "莲"],
    ["Squash", "窝瓜", "倭瓜", "窝瓜大叔", "倭瓜大叔", "镇压者"],
    ["Threepeater", "三重射手", "三线射手", "三头豌豆", "三联装豌豆", "三重", "三线", "三头", "三管", "管"],
    ["Tangle Kelp", "缠绕水草", "缠绕海草", "缠绕海藻", "缠绕海带",
        "水草", "海草", "海藻", "海带", "马尾藻", "绿毛线", "毛线"],
    ...
    ["Garlic", "大蒜", "蒜"],
    ["Umbrella Leaf", "萝卜伞", "叶子保护伞", "伞型保护叶", "莴苣",
        "萝卜", "白菜", "保护伞", "叶子伞", "伞叶", "叶子", "伞", "叶"],
...
# 模仿者卡片前缀
seeds_imitater_string = ["Imitater", "imitater",
                         "变身茄子", "模仿者", "模仿", "复制", "白", "小白", "克隆"]
